...
public protocol RowFormTextCompositeOutput: FormItemCompositeProtocol, RowCompositeVisibleSetting, RowCompositeValidationSetting {
  var visualisation: ALFB.Visualization {get set}
}

public class RowFormTextComposite: RowFormTextCompositeOutput {
  ...
  public func validate(value: ALValueTransformable) -> ALFB.ValidationState {
    ...
    case .closure(let model):
      ...
      result = start.check(value: value.transformForDisplay())
    case .phone(let regexp):
      start.link(with: PhoneMiddlewareValidation())
        .link(with: Phone3PartRegExpMiddlewareValidation(regexp: regexp))
      result = start.check(value: value.transformForDisplay())
    }
